...
void handleAlarma() {
  if (estadoSistema == 1) {
    ...
    // Al recibir /alarma desde la celda de Python, ejecuta los tonos de microsueño
    buzzerMicrosueño(); 
  } else {
...
void loop() {
  ...
  if (estadoBtnMQ3 == 1 && btnMQ3 == 0) {
    if (estadoSistema == 1) {
      ...
      while (millis() - inicioVentana < 7000) {
        ...
        // Imprime valor bruto (escala del límite) junto a los milivoltios físicos
        Serial.printf("[RÁFAGA] -> MQ-135: %d (%0.1fmV) | MQ-3 (Límite %d): %d (%0.1fmV)\n", 
  ...
  // ── MONITOR CONTINUO DE AIRE (Muestra el valor bruto para comparar con tu límite) ──
  if (estadoSistema == 1) {
    
    if (millis() - ultimoTiempoLectura >= intervaloLectura) {
      ultimoTiempoLectura = millis();

      int16_t lectura_MQ135 = ads.readADC_SingleEnded(0); // A0 -> Calidad de Aire
      float mv_continuo_aire = lectura_MQ135 * ADS_FACTOR_MV;

      // Imprime tanto las unidades del límite como los milivoltios
      Serial.printf("MONITOR SENSORES -> MQ-135 (Límite %d): %d unidades | Tensión: %0.1f mV\n", 
